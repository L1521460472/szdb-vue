 ...
 * @LastEditTime: 2024-01-15 15:52:21
-->
<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="tableData">
      ...
      <el-table-column label="附件" align="center" prop="invoiceFile">
        <!-- <template #default="scope">
          <el-image style="width: 100px; height: 50px" :src="scope.row.invoiceFile" :preview-src-list="[scope.row.invoiceFile]" preview-teleported/>
        </template> -->
        <template #default="scope">
          <span v-if="scope.row.invoiceFile" style="cursor: pointer;color: #409eff;">附件.{{ scope.row.invoiceFile.slice(scope.row.invoiceFile.lastIndexOf('.') + 1) }}</span>
          <el-image style="width: 100%; height: 23px;position: absolute;top: 12px;left: 0;z-index: 1;opacity: 0;" :src="scope.row.invoiceFile" :preview-src-list="[scope.row.invoiceFile]" preview-teleported/>
        </template>
      </el-table-column>
    ...
    >
      <el-form ref="invoiceRef" :model="formInfo.data" :rules="formInfo.rules" label-width="90px">
        <el-row>
          <el-col :span="24">
            <el-form-item label="项目名称" prop="projectId">
              <el-select v-model="formInfo.data.projectId" @change="handleChange" placeholder="项目名称" filterable clearable>
                <el-option
                  v-for="item in listTypeInfo.projectList"
                  :key="item.projectId"
                  :label="item.projectName"
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="开票金额" prop="invoiceAmount">
              <el-input v-model.number="formInfo.data.invoiceAmount" placeholder="请输入开票金额" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="24">
            <el-form-item label="开票日期" prop="invoiceTime">
              <el-date-picker
                v-model="formInfo.data.invoiceTime"
                type="date"
                format="YYYY-MM-DD"
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="附件" class="fileUpload" prop="invoiceFile">
              <el-upload
                ref="uploadRef"

<template>
    <div class="app-container">
       ...
       <el-table v-loading="loading" :data="projectList" @selection-change="handleSelectionChange">
          ...
          <!-- <el-table-column label="商务人天" align="center" prop="postSort" /> -->
          <el-table-column label="单价" align="center" prop="unitPrice">
             <template #default="scope">
                <span>{{ scope.row.artsProjectFinance.unitPrice }}</span>
             </template>
          </el-table-column>
          <el-table-column label="订单金额" align="center" min-width="110" prop="orderAmount" show-overflow-tooltip>
             <template #default="scope">
                <span>{{ scope.row.artsProjectFinance.orderAmount }}</span>
             </template>
          </el-table-column>
          <el-table-column label="公司制作" align="center" prop="fabrication">
             <template #default="scope">
                <span v-if="scope.row.artsProjectFinance.fabrication == '1'">是</span>
                <span v-else>否</span>
             </template>
          </el-table-column>
          <el-table-column label="外包制作" align="center" prop="fabrication">
             <template #default="scope">
                <span v-if="scope.row.artsProjectFinance.fabrication == '2'">是</span>
                <span v-else>否</span>
             </template>
          </el-table-column>
          <el-table-column label="外包金额" align="center" prop="outsourcingAmount">
             <template #default="scope">
                <span>{{ scope.row.artsProjectFinance.outsourcingAmount }}</span>
             </template>
          </el-table-column>
          <el-table-column label="是否完成" align="center" prop="isContractor">
             <template #default="scope">
                <span v-if="scope.row.artsProjectFinance.isContractor == '1'">是</span>
                <span v-else>否</span>
             </template>
          </el-table-column>
 ...
 <script setup name="DevelopStatistics">
 import { listProject,deptList } from "@/api/money/orderStatistics";

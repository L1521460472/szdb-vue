 ...
 * @Version: 1.0.0
 * @Author: lijiancong
 * @Date: 2023-02-25 10:46:24
 * @LastEditors: lijiancong
 * @LastEditTime: 2023-03-03 18:52:31
-->
<template>
  <el-form
    class="my_form"
          ...
          >
            ...
            <span v-else-if="formType === 'views' || item.type === 'views'">
              <template v-if="item.list">
                <template v-for="childItem in listTypeInfo[item.list]">
                  {{childItem.value == data[item.value] ? childItem.key : ''}}
                </template>
              </template>
              <template v-else>
                {{data[item.value]}}
            ...
            <el-input
              ...
              onchange="value=value.replace(/^\s+|\s+$/g,'')"
            ...
            <div v-if="item.multiple?.value1" class="multiple_input">
              <el-input
                ...
                :size="item.size"
                type="input"
                :disabled="item.multiple.isEnable? !item.multiple.isEnable : true"
              />
              <el-input
...
let tmpList = [];
const selectEvent = (event, item, data) => {
  if(data.countryCode){
    if(data.cityCode){
      data.cityCode = null
 ...
 * @param {*} item 表单项数据
 */
const getPlaceholder = (item, data) => {
  let placeholder = "";
 ...
 * @param {*} item 下拉选择项
 */
const getLabelValues = (item, data) => {
  if (
 ...
 * @param {*} item 下拉选择项
 */
const handleEvent = (event, data, item) => {
  getLabelValues(item, data);

...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const list = ref(props.defaultList);
    const arr = ref(props.defaultList);
     ...
     * @param queryText 查询文本
     */
    async function listApi(queryText) {
      // 只查启用状态的数据
      let data = {
        conditionList: [
          {
      ...
      if (props.defaultParams) {
        if (props.defaultParams.paymentType) {
          ...
          let arr = [];
          let res = await service['post'](props.listUrl, param);
          if (res) {
            arr = res.map((item)=>{
        ...
        if (props.defaultParams.measureUnit) {
          let arr = [];
          let res = await service[props.httpMethod](props.listUrl, data);
          if(res) { 
            arr = res.map((item)=>{
    ...
    function remoteMethod(val){
      ...
      list.value = arr.value.filter((item) => {
        // console.log(val)
        if(item.code){
